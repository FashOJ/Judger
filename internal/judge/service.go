...
func NewJudgeService(workers int, queueSize int) *JudgeService {
	...
	// 使用配置中的 CgroupRoot
	pool, err := sandbox.NewCgroupPool(workers, config.GlobalConfig.Sandbox.CgroupRoot)
	if err != nil {
...
func (s *JudgeService) runTestCase(ctx context.Context, exePath string, task *model.JudgeTask, tc model.TestCase) model.CaseResult {
	...
	if status != model.StatusAccepted {
		...
		case model.StatusRuntimeError:
			caseRes.Message = limitString(stderr, 500)
		case model.StatusTimeLimitExceeded:
			caseRes.Message = "Time Limit Exceeded"
		default:
			// 其他状态保持默认
		}
		return caseRes
